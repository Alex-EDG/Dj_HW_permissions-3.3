...
class AdvertisementViewSet(viewsets.ModelViewSet):
    ...
    def get_favorite(self, request, pk=None):
        ...
        if created:
            return Response(
                {'message': f'Объявление с id={pk} добавлено в избранное пользователя с id={request.user.id}'},
                status=status.HTTP_201_CREATED
            )
        else:
            favorite_obj.delete()
            return Response(
                {'message': f'Объявление с id={pk} удалено из избранного пользователя с id={request.user.id}'},
